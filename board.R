...
get_participants <- function() {
  names <- dir("Answers/")
  map(names, ~c(Nombre = .x, Valuaciones = length(dir(glue("Answers/{.x}"), pattern = ".csv")))) %>% 
    bind_rows() %>% 
    arrange(Nombre)
}
...
server <- function(input, output, session) {
  ...
  observeEvent(
    answers(),
    updateSelectInput(
      session, "answer_selector", choices = answers()$Producto,
      selected = if_else(
